...
def GetBaseFlags(filename):
    if IsSourceFile(filename):
        extension = os.path.splitext(filename)[1]
        if extension in C_SOURCE_EXTENSIONS:
            return C_BASE_FLAGS
        else:
            return CPP_BASE_FLAGS + CPP_SYS_INCLUDE
    return []
...
def FlagsForProject(root):
    try:
        project_path = FindNearest(root, '.clang_flags')
        project_flags = open(project_path, 'r').read().splitlines()
...
def Settings(filename, **kwargs):
    ...
    if compilation_db_flags:
        final_flags = compilation_db_flags + GetBaseFlags(filename)
    else:
        final_flags = GetBaseFlags(filename)
        include_flags = FlagsForInclude(root)
        if include_flags:
            final_flags = final_flags + include_flags
    # Get any additional project specific flags
    clang_flags = FlagsForProject(root)
    if clang_flags:
