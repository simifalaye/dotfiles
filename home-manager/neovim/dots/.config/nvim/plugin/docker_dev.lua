...
local function validate_conf(conf)
	...
	for _, v in ipairs({
		...
		if not conf[v.name] or type(conf[v.name]) ~= v.type then
			vim.schedule(function()
				vim.notify(
					string.format("Docker-dev: key '%s' missing or unexpected type (%s)", v.name, v.type),
	...
	for _, v in ipairs(conf.mounts) do
		if type(v) ~= "string" then
			vim.schedule(function()
				vim.notify("All mounts must be a string of the form 'local_path:remote_path'", vim.log.levels.ERROR)
			end)
			return false
		end
	end
	...
	for _, v in ipairs(conf.cmd) do
		if type(v) ~= "string" then
			vim.schedule(function()
				vim.notify("All cmd elements must be a string", vim.log.levels.ERROR)
			end)
			return false
		end
	end
...
function DockerDev.container_start(name, image, opts)
	...
	local containerStatus = vim.fn.systemlist("docker ps --format '{{.Names}}' --filter name=" .. name)
	if #containerStatus == 0 then
		...
		for _, v in ipairs(opts.mounts) do
			if v.type and v.source and v.target then
				mount_str = mount_str
					.. string.format("--mount type=%s,source=%s,target=%s ", v.type, v.source, v.target)
			end
		end
		...
		if exit_code == 0 then
			vim.notify("Dev container started: " .. name, vim.log.levels.INFO)
			return true
		else
			vim.notify("Dev container failed to start: " .. exit_code, vim.log.levels.ERROR)
			return false
		end
	else
		vim.notify("Dev container already running", vim.log.levels.DEBUG)
		return true
	end
end
...
function DockerDev.container_wait(name, cmd, opts)
	...
	local function run()
		...
		local command =
			string.format("docker exec --user %s %s sh -c '%s'", string.format("%s:%s", opts.uid, opts.gid), name, cmd)
		local _ = vim.fn.jobstart(command, {
			on_exit = function(_, exit_code)
				if exit_code == 0 then
					vim.notify(string.format("Dev container ready: %s", name), vim.log.levels.DEBUG)
					opts.cb()
				else
					vim.notify(
					...
					if attempts < opts.max_attempts or opts.max_attempts == 0 then
						...
						vim.notify(
							string.format("Dev container never became ready: %s, attempts: %d", name, attempts),
							vim.log.levels.ERROR
						)
					end
				end
	...
	DockerDev.container_wait(dconfig.container_name, dconfig.container_ready_cmd, {
		...
		cb = function()
			...
			if not client_id then
				vim.notify("Failed to start docker dev client", vim.log.levels.ERROR)
			end
			-- Setup user commands
...
function DockerDev.start(config, opts)
	...
	if dconfig == nil then
		local conf_path = vim.fs.joinpath(config.root_dir, DOCKER_DEV_FILE)
		local stat = vim.uv.fs_stat(conf_path)
		if not stat then
			vim.notify("No docker dev config at: " .. conf_path, vim.log.levels.DEBUG)
			return false
		end
...
function DockerDev.get_config(root_dir)
  return _G.DockerDev.config_cache[root_dir]
end
...
local augroup = vim.api.nvim_create_augroup("docker-dev", { clear = true })
vim.api.nvim_create_autocmd({ "BufWinEnter", "FileChangedShellPost" }, {
  group = augroup,
	desc = "Load docker-dev config",
	pattern = "*",
	callback = function(info)
    if not vim.g.user_docker_dev_enabled then
      return
    end
    ...
    local root_dir = vim.fs.root(info.file, DOCKER_DEV_FILE)
    ...
    -- Read and validate conf file
    config = json.read(vim.fs.joinpath(root_dir, DOCKER_DEV_FILE))
