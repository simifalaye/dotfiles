...
#-
#  Common config helper functions
#-
...
include () {
    for src in "$@"; do
        ...
        test -r "$src" && . "$src" || echo "Failed to source: $src"
    done
}
...
# Explicitly set XDG base dirs as they are needed later and not always set
