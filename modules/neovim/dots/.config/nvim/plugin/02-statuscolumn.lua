...
---@param data user.stc.shared_data
---@param filter fun(sign: user.extmark.sign, data: user.stc.shared_data): boolean
...
function builders.signcol(data, filter, virtual)
  ...
  return utils.hl.make_hl_str(' ', data.culhl and 'CursorLineSign' or 'SignColumn')
end
...
function builders.foldcol(data)
  ...
  local foldchar = (data.virtnum ~= 0 or foldinfo.start ~= lnum)
    ...
    or data.foldclose
  return utils.hl.make_hl_str(foldchar, data.culhl and 'CursorLineFold' or 'FoldColumn')
end

---Get a valid name of an extmark sign
---@param sign user.extmark.sign
---@return string
...
function _G.UserStatusColumn()
  ...
  local signs = builders.signcol(data, nongitsigns_filter)
  local function is_empty_segment(s)
    -- highlight set
    s = s:gsub("%%#.-#", "")
    -- highlight reset
    s = s:gsub("%%%*", "")
  ...
  return (is_empty_segment(signs) and builders.foldcol(data) or signs)
...
local augroup = vim.api.nvim_create_augroup('user.statuscolumn', {})
...
vim.o.statuscolumn = "%!v:lua.UserStatusColumn()";
