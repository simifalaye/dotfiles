--
-- Add readline mappings to neovim insert mode
-- Source: https://github.com/Bekaboo/dot/blob/master/.config/nvim/lua/plugin/readline.lua
--

if vim.g.loaded_user_pluginreadline then
  return
end
vim.g.loaded_user_pluginreadline = true

local regex_keyword_at_beginning = vim.regex([=[^\s*[[:keyword:]]*]=])
local regex_nonkeyword_at_beginning = vim.regex([=[^\s*[^[:keyword:][:space:]]*]=])
...
local function get_current_line()
  return vim.fn.mode() == "c" and vim.fn.getcmdline() or vim.api.nvim_get_current_line()
end
...
vim.keymap.set(
  ...
  "<C-BS>",
  "<C-w>",
  { remap = true, desc = "Delete word before cursor" }
)
vim.keymap.set(
  "!",
  "<M-BS>",
...
vim.keymap.set("!", "<M-d>", function()
  return small_del(end_of_line() and not last_line() and "\n" or get_word_after(), true)
